...
def eval_mriqc(subject_dir, session_id, run_id=None):
    ...
    output_log = f'{subject_dir}/mriqc_out_{str(subject_id)}.log'
    f = open(output_log, 'r')
    ...
    if "Participant level finished successfully." in f.read(): 
        ...
        #create path string
        acq_T1w = f'{subject_dir}/{str(subject_id)}_ses-{str(session_id)}*_run-*_T1w*'
...
def check_bold(subject_dir, session_id, run_id=None):
    ...
    if "Participant level finished successfully." in f.read(): 
        ...
        #create path string
        acq_bold = f'{subject_dir}/{subject_id}_ses-{session_id}*_task-rest_run-*_bold*'

    	#check if string is existing path
        return 'SUCCESS' if glob.glob(acq_bold) else 'FAIL'
    else: #no sign participant passed, assume failure for all datatypes
        return 'FAIL'
