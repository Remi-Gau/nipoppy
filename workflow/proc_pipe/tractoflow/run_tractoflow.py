...
def parse_data(bids_dir, participant_id, session_id, logger=None):
    ...
    for idx, dmri in enumerate(dmri_files):
        ...
        tbvec = Path(
            ...
            f'ses-{session_id}',
            'dwi',
            dmri.filename.replace('.nii.gz', '.bvec'),
        ).joinpath()
        tbval = Path(
            bids_dir,
            participant_id,
            f'ses-{session_id}',
            'dwi',
            dmri.filename.replace('.nii.gz', '.bval'),
        ).joinpath()
    ...
    if sum(cbv == 1) > 1:
            ...
            if (dmrifs1nv == dmrifs2nv):
                ...
                rpeb0s = np.loadtxt(
                    Path(
                        ...
                        f'ses-{session_id}',
                        'dwi',
                        dmrifs2.filename.replace('.nii.gz', '.bval'),
                    ).joinpath()
                )
        ...
        rpe = f"{fpe}-" if (len(fpe) == 1) else fpe[0]
    ...
    bvalfile = Path(
        ...
        f'ses-{session_id}',
        'dwi',
        dmri_files[didx].filename.replace('.nii.gz', '.bval'),
    ).joinpath()
    bvecfile = Path(
        bids_dir,
        participant_id,
        f'ses-{session_id}',
        'dwi',
        dmri_files[didx].filename.replace('.nii.gz', '.bvec'),
    ).joinpath()
    anatfile = Path(
        bids_dir, participant_id, f'ses-{session_id}', 'anat', oanat.filename
    ).joinpath()

    ## condition empty path
    rpe_file = None if rpe_out is None else Path(tmp_dir, rpe_out).joinpath()
